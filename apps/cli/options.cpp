...
std::string usage_text(const char* argv0) {
    return std::string("usage: ") + argv0 +
           ...
           "       [--reasoning-effort low|medium|xhigh] [--vision] [--embed-cpu]\n"
           "       [--no-cuda-graph]\n"
           "\n"
           ...
           "--embed-cpu keeps the token embedding table in pinned host RAM; the GPU gathers the\n"
           "needed rows over PCIe (frees the table's GPU footprint). Works with all execution\n"
           "routes, including speculative decoding.\n"
