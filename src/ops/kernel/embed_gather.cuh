...
inline constexpr std::int32_t kEmbedGatherQ5Group          = 64;
inline constexpr std::int32_t kEmbedGatherQ5NibbleBpr      = 32;
inline constexpr std::int32_t kEmbedGatherQ5HighBpr        = 8;
inline constexpr std::int32_t kEmbedGatherQ5GroupsPerBlock = 2;
...
__device__ __forceinline__ int unpack_q5_code(const std::uint8_t* nibble,
                                               const std::uint8_t* high, int index) {
    const std::uint8_t low_byte = nibble[index >> 1];
    const std::uint32_t low =
        (index & 1) ? (low_byte >> 4) : (low_byte & 0x0fu);
    const std::uint32_t high_bit =
        (high[index >> 3] >> (index & 7)) & 0x01u;
    const std::uint32_t u = low | (high_bit << 4);
...
__global__ void embed_gather_q5_kernel(const std::int32_t* ids,
                                        const std::uint8_t* codes,
                                        const std::uint8_t* high,
                                        const std::uint8_t* scales,
                                        __nv_bfloat16* out, std::int32_t d,
                                        std::int32_t T, std::int32_t padded_d) {
    const std::int32_t kg = padded_d / kEmbedGatherQ5Group;
    const std::int64_t n = static_cast<std::int64_t>(d) * T;
    ...
    for (std::int64_t i = start; i < n; i += stride) {
        const std::int32_t t = static_cast<std::int32_t>(i / d);
        const std::int32_t k =
            static_cast<std::int32_t>(i - static_cast<std::int64_t>(t) * d);
        const std::int32_t row = ids[t];
        const std::int32_t g = k / kEmbedGatherQ5Group;
        const std::int32_t lane = k - g * kEmbedGatherQ5Group;
        ...
        const int code =
            unpack_q5_code(codes + group_index * kEmbedGatherQ5NibbleBpr,
                           high + group_index * kEmbedGatherQ5HighBpr, lane);

        out[i] = __float2bfloat16(static_cast<float>(code) * scale);
    }
}

__launch_bounds__(kEmbedGatherQ5Group * kEmbedGatherQ5GroupsPerBlock)
__global__ void embed_gather_q5_grouped_kernel(
    const std::int32_t* ids, const std::uint8_t* codes,
    ...
    const std::int32_t t =
        static_cast<std::int32_t>(blockIdx.x) / group_blocks;
    const std::int32_t block_group =
        static_cast<std::int32_t>(blockIdx.x) - t * group_blocks;
    const std::int32_t group_slot =
        threadIdx.x / kEmbedGatherQ5Group;
    const std::int32_t lane =
        threadIdx.x - group_slot * kEmbedGatherQ5Group;
    const std::int32_t g =
        block_group * kEmbedGatherQ5GroupsPerBlock + group_slot;
    ...
    if ((lane & 31) == 0) {
        scale_bits =
            static_cast<int>(scales[group_index * 2]) |
            (static_cast<int>(scales[group_index * 2 + 1]) << 8);
    }
    scale_bits = __shfl_sync(0xffffffffu, scale_bits, 0);

    const float scale =
        __half2float(__ushort_as_half(static_cast<std::uint16_t>(scale_bits)));
    ...
    const std::int64_t out_idx =
        ...
        static_cast<std::int64_t>(g) * kEmbedGatherQ5Group + lane;

    out[out_idx] =
        __float2bfloat16(static_cast<float>(code) * scale);
}
