...
void gqa_attention_prefill_fill_q4_kernel(
    ...
    const std::int64_t src0 =
        gqa_kv_quant_src_index<Geometry>(kv_head, d0, token);
    const std::int64_t src1 =
        gqa_kv_quant_src_index<Geometry>(kv_head, d1, token);
    ...
    const __half ksh =
        __float2half_rn(k_abs > 0.0f ? k_abs / 7.0f : 0.0f);
    const __half vsh =
        __float2half_rn(v_abs > 0.0f ? v_abs / 7.0f : 0.0f);
    ...
    const std::int64_t code_off =
        gqa_kv_q4_code_index<Geometry>(page, kv_head, d0, page_off);

    cache_k[code_off] =
        gqa_kv_pack_q4(
            gqa_kv_quant_q4_code(k0, kinv),
            gqa_kv_quant_q4_code(k1, kinv));

    cache_v[code_off] =
        gqa_kv_pack_q4(
            gqa_kv_quant_q4_code(v0, vinv),
            gqa_kv_quant_q4_code(v1, vinv));
...
template <typename Geometry, typename Metadata>
__launch_bounds__(256) __global__
void gqa_attention_prefill_fill_q4_page_kernel(
    const __nv_bfloat16* __restrict__ k,
    const __nv_bfloat16* __restrict__ v,
    const std::int32_t* __restrict__ positions,
    Metadata metadata,
    ...
    const int tile_position =
        (base_position / TokensPerTile + tile_delta) * TokensPerTile;

    const int logical_page =
        tile_position >> kPagedKVPageShift;

    const int token_begin =
        max(0, tile_position - base_position);

    const int token_end =
        min(tokens, tile_position + TokensPerTile - base_position);
    ...
    int physical_page =
        lane == 0 ? block_table[logical_page] : 0;
    ...
    const float k_abs =
        warp_max(fmaxf(fabsf(k0), fabsf(k1)), FullMask);
    const float v_abs =
        warp_max(fmaxf(fabsf(v0), fabsf(v1)), FullMask);
    ...
    physical_page =
        __shfl_sync(FullMask, physical_page, 0);
    ...
    const std::int64_t code_off =
        gqa_kv_q4_code_index<Geometry>(
            physical_page, kv_head, d0, page_off);
    ...
    if (lane == 0) {
        const std::int64_t scale_off =
            gqa_kv_quant_scale_index<Geometry>(
                physical_page, kv_head, group, page_off);
...
template <typename Geometry, typename Metadata>
__global__ __maxnreg__(120) void gqa_attention_prefill_q4_kernel(
    ...
    auto issue_kv_tile = [&](int tile_k0) {
        ...
        // Scales remain identical in layout to INT8-G64.
        ...
        // Global Q4 uses 8 packed bytes for each 16-dimension chunk.
        // Expand those 16 signed nibble values into the exact INT8 shared
        // tile layout consumed by the existing mma_s8 QK path.
#pragma unroll 1
        for (int chunk = tid; chunk < Bc * (D / 16);
            ...
            if (key <= max_query_abs) {
                const std::int64_t off =
                    gqa_kv_q4_code_index<Geometry>(
                        physical_page, kv_head, d, key_l);

                const int2 kraw = load_vec<int2>(&cache_k[off]);
                const int2 vraw = load_vec<int2>(&cache_v[off]);
...
#pragma unroll
                for (int i = 0; i < 8; ++i) {
                    kd[2 * i] =
                        gqa_kv_unpack_q4_low(kb[i]);
                    kd[2 * i + 1] =
                        gqa_kv_unpack_q4_high(kb[i]);

                    vd[2 * i] =
                        gqa_kv_unpack_q4_low(vb[i]);
                    vd[2 * i + 1] =
                        gqa_kv_unpack_q4_high(vb[i]);
                }
            } else {
        ...
        // Only the scale copies above are asynchronous.
