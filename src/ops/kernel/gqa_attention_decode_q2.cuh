...
// agree.
__device__ __forceinline__ void gqa_small_t_q2_store_swz(std::int8_t* tile, int row, int d,
...
__launch_bounds__(WarpsPerCta * 32, MinBlocksPerSm) __global__
    void gqa_attention_decode_q2_tiled_kernel(
        const __nv_bfloat16* q, CacheInput input, const std::int32_t* pos, std::uint8_t* cache_k_q2,
        std::uint8_t* cache_v_q2, __half* cache_k_scale, __half* cache_v_scale,
    ...
    constexpr bool AsyncPackedV = false;

    // Stage packed K asynchronously alongside V.
    constexpr bool AsyncPackedK = false;
    ...
    for (int unit = warp; unit < RowCount * Groups; unit += Wc) {
        ...
        gqa_small_t_q2_store_swz(q_i8, row, d0, DB16, gqa_kv_quant_code(x0, inv));
        gqa_small_t_q2_store_swz(q_i8, row, d1, DB16, gqa_kv_quant_code(x1, inv));
            ...
            const int key   = tile_k0 + key_l;
                  ...
                  if constexpr (AsyncPackedK) {
                      ...
                      ninfer::ops::cp_async<8>(
                          kdst_q4, &cache_k_q2[off]);
                  } else {
                      const std::uint32_t kraw =
                          static_cast<std::uint32_t>(
                              load_vec<int>(&cache_k_q2[off]));
...
#pragma unroll
                  ...
                  if constexpr (AsyncPackedV) {
                      ...
                      ninfer::ops::cp_async<8>(
                          vdst_q4, &cache_v_q2[off]);
                  } else {
                      std::int8_t* vdst =
                          &v_i8[key_l * D + d];
                      const std::uint32_t vraw =
                          static_cast<std::uint32_t>(
                              load_vec<int>(&cache_v_q2[off]));
...
#pragma unroll
                    ...
                    float kb2      = 0.0f;
                    if (gid == 0) {
                        ka  = __half2float(k_scale_s[keya * Groups + g]) / 3.0f;
                        kb2 = __half2float(k_scale_s[keyb * Groups + g]) / 3.0f;
                    }
...
#pragma unroll 1
            for (int chunk = loader_tid; chunk < Bc * (D / 8); chunk += VLoaderThreads) {
                ...
                if (key >= split_start && key < split_end) {
                    const int grp = d >> 6;
                    float vs      = 0.0f;
                    if ((lane & 7) == 0) { vs = __half2float(v_scale_s[key_l * Groups + grp]) / 3.0f; }
