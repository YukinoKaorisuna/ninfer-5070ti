...
// agree.
__device__ __forceinline__ void gqa_small_t_q4_store_swz(std::int8_t* tile, int row, int d,
...
__launch_bounds__(WarpsPerCta * 32, MinBlocksPerSm) __global__
    void gqa_attention_decode_q4_tiled_kernel(
        const __nv_bfloat16* q, CacheInput input, const std::int32_t* pos, std::uint8_t* cache_k_q4,
        std::uint8_t* cache_v_q4, __half* cache_k_scale, __half* cache_v_scale,
    ...
    for (int unit = warp; unit < RowCount * Groups; unit += Wc) {
        ...
        gqa_small_t_q4_store_swz(q_i8, row, d0, DB16, gqa_kv_quant_code(x0, inv));
        gqa_small_t_q4_store_swz(q_i8, row, d1, DB16, gqa_kv_quant_code(x1, inv));
