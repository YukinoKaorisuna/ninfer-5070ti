...
// Folded gate/up Q3G64 x INT8-activation Tensor Core GEMM for prefill token counts.
//
...
// contract exactly: Q3G64 codes are consumed as INT8 without a floating-point
...
// measurements behind it, including why this rescale and not the packed-weight decode is
...
struct Q3Int8SwiGluSchedule {
    ...
    static constexpr int kSuperK         = kGroupsPerSuper * kGroupK; // 256
    static constexpr int kCodeRowStride  = kGroupsPerSuper * kCodeBytesPerGroup; // 96
    static constexpr int kChunksPerRow   = kCodeRowStride / 16; // 6

    static constexpr int kBlockK  = kGroupK; // 64
    ...
    static_assert(kSharedBytes <= 99 * 1024, "Q3 INT8 SwiGLU exceeds the dynamic shared budget");
};

// Load four consecutive Q3 values from one packed G64 group.
//
// Each value is signed 3-bit two's complement. Four values occupy 12 bits.
// `first_code` is a multiple of four in the MMA mapping, therefore the
// starting bit is byte-aligned or +4 bits and two bytes always contain the
// complete 12-bit payload.
__device__ __forceinline__
std::uint32_t q3_load_packed12(
    const std::uint8_t* __restrict__ group_codes,
    int first_code) {

    const int bit   = first_code * 3;
    const int byte  = bit >> 3;
    const int shift = bit & 7;

    const std::uint32_t bits =
        static_cast<std::uint32_t>(group_codes[byte]) |
        (static_cast<std::uint32_t>(group_codes[byte + 1]) << 8);

    return (bits >> shift) & 0x0fffu;
}

// clang-format off
template <class Schedule_, bool Full>
__global__ __launch_bounds__(Schedule_::kThreads, Schedule_::kMinBlocks)
void q3_linear_swiglu_int8_gemm_kernel(
    ...
    auto cr_at = [&](int st, int local_row, int chunk) {
        return &Cr[(st * BM + local_row) * CRS + chunk * 16];
    };
...
#pragma unroll 1
        for (int item = tid; item < BM * CPR; item += S::kThreads) {
            ...
            if (Full || m0 + (local_row & (PM - 1)) < intermediate) {
                ...
                cp_async<16>(dst,
                             &codes[gi * S::kCodeBytesPerGroup + chunk * 16]);
            } else {
...
#pragma unroll
                for (int mi = 0; mi < MT; ++mi) {
                    const int r0 =
                        tile_row(mi) + group_row;

                    const std::uint8_t* g0 =
                        &Cr[
                            (wst * BM + r0) * CRS
                            + j * S::kCodeBytesPerGroup];
