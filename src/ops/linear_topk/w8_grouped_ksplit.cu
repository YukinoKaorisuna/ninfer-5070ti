...
__launch_bounds__(Schedule::kThreads, Schedule::kMinBlocksPerSm) void w8_grouped_ksplit_topk_kernel(
    ...
    constexpr int kRowTiles  = kLinearTopKMRowsPerGroup / kRows;
    constexpr unsigned kMask = 0xffffffffu;
    static_assert(kRows == 16 && kLinearTopKMRowsPerGroup == 128);
    ...
    for (int row_tile = 0; row_tile < kRowTiles; ++row_tile) {
        const int row_begin =
            static_cast<int>(blockIdx.x) * kLinearTopKMRowsPerGroup + row_tile * kRows;
...
struct GroupedSchedule {
    using Type = typename W8LinearSmallTProductionSchedule<W8VocabularyProjectionGeometry,
                                                           ActiveColumns>::Type;
};

template <>
struct GroupedSchedule<35> {
    using Type = W8SmallTMmaSchedule<4, 40, 2, W8SmallTMmaScaleAccess::Shared>;
};
...
void launch_exact(const Tensor& hidden, const Weight& head, std::int32_t valid_rows,
    ...
    using Schedule = typename GroupedSchedule<ActiveColumns>::Type;
...
constexpr auto make_launchers(std::index_sequence<Indices...>) {
    return std::array<Launch, sizeof...(Indices)>{&launch_exact<14 + 7 * Indices>...};
}

constexpr auto kLaunchers = make_launchers(std::make_index_sequence<2>{});

} // namespace

void linear_topk_w8_grouped_ksplit_launch(const Tensor& hidden, const Weight& head,
                                          std::int32_t valid_rows,
                                          const LinearTopKWorkspace& workspace,
                                          cudaStream_t stream) {
    const std::size_t index = static_cast<std::size_t>(hidden.ne[1] / kLinearTopKWidth - 2);
    kLaunchers[index](hidden, head, valid_rows, workspace, stream);
}
