...
void launch_exact(const Tensor& hidden, const Weight& head, std::int32_t valid_rows,
    ...
    using Schedule = typename Fp8VocabularyA16SmallTMmaProductionSchedule<ActiveColumns>::Type;
...
void linear_topk_fp8_grouped_ksplit_launch(const Tensor& hidden, const Weight& head,
