...
Options parse_args(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (!std::strcmp(argv[index], "--warmup")) {
            ...
            std::printf("usage: %s [--k 4096|17408] [--batch 1..8] [--warmup N] [--repeat N] "
                        "[--flush-mib N]\n",
                        argv[0]);
            std::exit(0);
        } else {
...
void run_profile(std::int32_t input_rows, const Options& options, DeviceBuffer& flush,
                 cudaStream_t stream) {
    DeviceBuffer input    = make_bf16(static_cast<std::size_t>(input_rows) * kWidth * 8);
    DeviceBuffer base     = make_bf16(static_cast<std::size_t>(kHidden) * kTaps * kSides);
    DeviceBuffer delta    = make_bf16(static_cast<std::size_t>(kGroups) * kTaps * kWidth * 8);
    ...
    const std::size_t capacity =
        ops::linear_dynamic_grouped_conv_add_workspace_capacity_bytes(input_rows, 1, 8);
    ...
    for (std::int32_t batch_size = 1; batch_size <= 8; ++batch_size) {
        ...
        const std::size_t exact_capacity =
            ops::linear_dynamic_grouped_conv_add_workspace_capacity_bytes(input_rows, batch_size,
                                                                          batch_size);
        Tensor x(input.p, DType::BF16, {input_rows, kWidth, batch_size});
        Tensor finish_delta(delta.p, DType::BF16, {kGroups, kTaps, kWidth, batch_size});
        Tensor residual_view(residual.p, DType::BF16, {kHidden, kWidth, batch_size});
        ...
        const auto measure = [&](const char* route, auto&& launch) {
            ...
            std::printf("%d,%d,%d,%s,%.3f,%.3f,%.3f,%.2f,%.1f,%zu,%zu\n", input_rows, batch_size,
                        cols, route, timing.median_us, timing.min_us, timing.p95_us, tflops,
                        bandwidth, graph.nodes(), exact_capacity);
        };
        measure(ops::detail::w8_linear_dynamic_grouped_conv_add_route_name(input_rows, batch_size),
                [&](cudaStream_t capture_stream) {
                    ops::linear_dynamic_grouped_conv_add(x, packed.weight, base_kernel,
                                                         finish_delta, residual_view, workspace,
                                                         capture_stream);
                });
    }
}
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        std::printf("C,B,T,route,median_us,min_us,p95_us,effective_tflops,effective_gbs,"
