...
Options parse_options(int argc, char** argv) {
    ...
    for (int index = 1; index < argc; ++index) {
        ...
        if (!std::strcmp(argv[index], "--batch")) {
            ...
            std::printf("usage: %s [--batch 1..8] [--warmup N] [--repeat N] "
                        "[--flush-mib N]\n",
                        argv[0]);
            std::exit(0);
        } else {
...
struct Fixture {
    std::array<std::int32_t, kCandidates * kSteps * 8> host_candidates{};
    std::array<float, kCandidates * kSteps * 8> host_unary{};
    ...
    DeviceBuffer projected_hidden = make_bf16(static_cast<std::size_t>(kRank) * kSteps * 8);
    ...
    DeviceBuffer drafts{static_cast<std::size_t>(kSteps) * 8 * sizeof(std::int32_t)};
    DeviceBuffer proposal_q{static_cast<std::size_t>(kCandidates) * kSteps * 8 * sizeof(float)};
...
void measure(const char* route, std::int32_t batch_size, Mode mode, const Options& options,
             DeviceBuffer& flush, cudaStream_t stream, Launch&& launch) {
    TimedGraph graph;
    graph.capture(stream, launch);
    const ColdTiming timing =
        measure_cold_graph(graph, flush, stream, options.warmup, options.repeat);
    std::printf("%d,%s,%s,%.3f,%.3f,%.3f,%zu,0\n", batch_size, mode_name(mode), route,
...
void run(std::int32_t batch_size, Mode mode, const Options& options, Fixture& fixture,
    ...
    measure(ops::detail::candidate_selector_path_route_name(batch_size), batch_size, mode, options,
            flush, stream, [&](cudaStream_t capture_stream) {
                ops::candidate_selector_path(ids, unary, hidden, anchor, predecessor, successor,
                                             positions, configs, draft, q, capture_stream);
            });
}
...
int main(int argc, char** argv) {
    try {
        ...
        std::printf("# gpu=%s public=candidate_selector_path geometry=C16_K7_R256 "
                    "cache=cold flush_mib=%zu execution=graph\n",
                    properties.name, options.flush_bytes >> 20);
        std::printf("B,mode,route,median_us,min_us,p95_us,graph_nodes,workspace_bytes\n");
