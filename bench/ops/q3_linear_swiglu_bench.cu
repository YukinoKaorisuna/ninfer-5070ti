// Cold-cache public Op benchmark for the registered Q3 LinearSwiGLU profile.
...
int main(int argc, char** argv) {
    try {
        ...
        bench::PackedQuantizedWeight packed = bench::make_row_split_weight(
            QType::Q3G64_F16S, kGateUpRows, kHidden, kHidden, {0x31, 0x00, 0x3c00});
        const std::size_t workspace_capacity = ops::linear_swiglu_workspace_capacity_bytes(
            QType::Q3G64_F16S, kGateUpRows, kHidden,
            ops::LinearPolicy::AllowA8, min_t, max_t);
        ...
        const auto launch = [&](std::int32_t tokens, cudaStream_t launch_stream) {
            ...
            ops::linear_swiglu(x, packed.weight, out,
        ...
        if (options.profile) {
            ...
            std::printf("PROFILE linear_swiglu Q3 T=%d workspace=%zu\n", options.tokens.front(),
                        workspace_capacity);
        ...
        std::fprintf(stderr, "ninfer_q3_linear_swiglu_bench: %s\n", error.what());
        return 1;
    }
}
